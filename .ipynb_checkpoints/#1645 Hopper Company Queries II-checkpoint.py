...
# Available drivers=cumm sum of drivers as of x month
# Drivers with rides requested_at x month
...
drivers_joined as 
(select month
 ,count(distinct driver_id) as n_drivers_joined
from months a
left join (select * from Drivers where YEAR(join_date)<=2020) b
on a.month=(case when YEAR(join_date)<2020 then 1 else Month(b.join_date) end)
 ...
 select Month(requested_at) as month,count(distinct driver_id) as n_drivers_active
     from Rides a
...
select a.month,
round(coalesce(n_drivers_active/sum(n_drivers_joined) over(order by month ASC)*100,0),2) as working_percentage

from drivers_joined a
left join drivers_accepted b
on a.month=b.month
