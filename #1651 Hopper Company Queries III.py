...
ref: https://www.mysqltutorial.org/mysql-recursive-cte/
...
ttls as (select MONTH(R.requested_at) as month,
sum(ride_distance)  as ttl_ride_distance ,
sum(ride_duration)  as ttl_ride_duration 
...
select a.month, 
round((sum(coalesce(ttl_ride_distance,0)) over (order by a.month ASC ROWS between CURRENT ROW  and 2 FOLLOWING))/3,2) as average_ride_distance,
round((sum(coalesce(ttl_ride_duration,0)) over (order by a.month ASC ROWS between CURRENT ROW  and 2 FOLLOWING))/3,2) as average_ride_duration
